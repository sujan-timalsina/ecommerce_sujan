    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                # open an atomic transaction, i.e. either all successful or none
            ...
            request.session["message"] = f"Successfull Payment of Rs. {amount} from your balance!"
...
def maintain_invoice(request, token, amount):
    ...
    # adjust quantity and clear cart
    for cart_item in cart_items:
        ...
        if product.quantity < cart_item.quantity:
            raise Exception(
                f"Insufficient Quantity: {cart_item.quantity} for {product.name}")
